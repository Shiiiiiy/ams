<table class="table table-bordered table-striped tablecut" id="selectUserTable">
	<thead>
		<tr>
			<th class="table_checkbox" width="3%"><input type="checkbox" name="_select_rows" class="select_rows" id="checkEduClassAll" onclick="checkEduClassAll(this);"/></th>
			<th  width="20%">名称</th>
			<th  width="15%">班号</th>
			<th  width="15%">年级</th>
			<th width="15%">班主任</th>
	...
	<tbody>
	<#if page??>
		<#list page.result as classes>
			<tr>
			    <td><input type="checkbox" name="row_sel_multi"  value="${(classes.classInfo)!""}" <#if selectedId?? && selectedId == classes.id>checked="true"</#if> onclick="_onClickEduClassCheckbox(this)"/></td>
				<td class="autocut">${(classes.className)!""}</td>
				<td class="autocut">${(classes.code)!""}</td>
				<td class="autocut">${(classes.grade)!""}</td>
				<td class="autocut">${(classes.headermaster.name)!""}</td>
				<td class="autocut">${(classes.major.collage.name)!""}</td>
				<td class="autocut">${(classes.major.majorName)!""}</td>
			</tr>
		</#list> 
	</#if>															
	</tbody>
...
$("document").ready(function(){
	...
	pageForm.action="${rc.contextPath}/comp/class/nsm/queryClassCheckBox.do";
	...
	$("#pageFormId").append($("<input name='className' type='hidden' value='${classes.className!""}'/>"));
		...
		var pageFormOptions = {
            ...
            success: function(data){
             	$('input[name="row_sel_multi"]').each(function(){
					...
					var userTree = $.fn.zTree.getZTreeObj("_selectedClassTree");
