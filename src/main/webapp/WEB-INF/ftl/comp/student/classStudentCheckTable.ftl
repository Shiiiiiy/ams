<table class="table table-bordered table-striped tablecut" id="selectUserTable">
	<thead>
		<tr>
			<th class="table_checkbox" width="3%"><input type="checkbox" name="_select_rows" class="select_rows" id="checkClassStudentAll" onclick="checkClassStudentAll(this);"/></th>
			<th  width="15%">姓名</th>
			<th  width="15%">学号</th>
			<th  width="10%">性别</th>
			<th width="20%">班级</th>
	...
	<#if page??>
		<#list page.result as student>
			<tr>
			    <td><input type="checkbox" name="row_sel_student_multi"  value="${(student.studentInfo)!""}" <#if selectedId?? && selectedId == student.id>checked="true"</#if> onclick="_onClickClassStudentCheckbox(this)"/></td>
