...
<div class="modal hide fade" id="_selectStudentAllCheckModal">
	...
	<div class="span9">
		<div class="row-fluid">
			<div class="span12">
				<form  id="_studentAllCheckQuery"  action="${rc.contextPath}/comp/student/nsm/queryCheckGraduateStudent.do" method="post">
...
function initCheckCollege()
{
	$.ajax({
		...
		success:function(data){ 
		  ...
		  var collegeId=$("#deptId").val();	
		  if("-1"==collegeId){
		  		$("<option value=''>请选择</option>").appendTo("#_checkCollegeId");
	  	  }	
		  $.each(data, function(i,item){
			   if(item.id == collegeId){	
		 			$('<option value='+item.id+' selected>'+item.name+'</option>').appendTo("#_checkCollegeId");
