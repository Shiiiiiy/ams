<table class="table table-bordered table-striped tablecut" style="style="width:70%;"" id="selectMajorTable">
	<thead>
		<tr>
			<th  width="5%"><input type="checkbox" name="_select_rows" class="select_rows" id="checkEduMajorAll" onclick="checkEduMajorAll(this);"/></th>
			<th  width="30%">专业名称</th>
			<th  width="15%">专业号</th>
			<th  width="20%">学院</th>
		</tr>
	...
	<tbody>
	<#if page??>
		<#list page.result as majors>
			<tr>
			    <td><input type="checkbox" name="row_sel_multi_"  value="${(majors.majorInfo)!""}" <#if selectedId?? && selectedId == majors.id>checked="true"</#if> onclick="_onClickEduMajorCheckbox(this)"/></td>
				<td class="autocut">${(majors.majorName)!""}</td>
				<td class="autocut">${(majors.majorCode)!""}</td>
				<td class="autocut">${(majors.collage.name)!""}</td>
			</tr>
		</#list> 
	</#if>															
	</tbody>
...
<div class="row-fluid">
<div class="span12" style="margin-top:-19px;" id="_radioSelectMajorPage">
	<div class="nextP">
                    ...
                    <div class="dataTables_paginate paging_bootstrap pagetion">
                        <ul>
                            ...
                            <li style="height: 30px; line-height:30px;">
				            	<input id="gotoPageNumber_" style="width:20px; padding:1px;height:18px; line-height:18px; margin-bottom: 2px;vertical-align:center; text-align: center; " value="${page.currentPageNo}"/>
				            	<input id="gotoPageFormId_" type="hidden" value="pageFormId"/>
				            	<input id="gotoPageTotalPageCount_" type="hidden" value="${page.totalPageCount}"/>
			           		 </li>
...
$("document").ready(function(){
	...
	pageForm.action="${rc.contextPath}/comp/major/nsm/queryMajorCheckBox.do";
	...
	$("#pageFormId").append($("<input name='collage.id' type='hidden' <#if baseMajor.collage??> value='${baseMajor.collage.id!""}' </#if>/>"));
	$("#pageFormId").append($("<input name='className' type='hidden' value='${baseMajor.majorName!""}'/>"));
	...
	$("#_radioSelectMajorPage #gotoPageNumber_").keypress(function(event){
	
		var maxPageNo= $("#_radioSelectMajorPage #gotoPageTotalPageCount_").val()*1;
		var toPageNo= $("#_radioSelectMajorPage #gotoPageNumber_").val()*1;
