<table class="table table-bordered table-striped tablecut" id="selectUserTable">
	<thead>
		<tr>
			<th class="table_checkbox" width="5%"><input type="checkbox" name="select_rows" class="select_rows" id="checkTeacherAll" onclick="checkTeacherAll(this);"/></th>
			<th  width="15%">姓名</th>
			<th  width="20%">工号</th>
			<th  width="15%">性别</th>
			<th width="40%">单位</th>
	...
	<#if page??> 
		<#list page.result as teacher>
			<tr>
			    <td><input type="checkbox" name="row_teacher_multi"  value="${(teacher.teacherInfo)!""}" onclick="_onClickTeacherCheckbox(this)"/></td>
                ...
                <div class="span7" style="padding-right:10px;height:30px; line-height:30px;"">
                    <div class="dataTables_paginate paging_bootstrap pagetion">
                        <ul>
                            ...
                            <li>
                            <a class="arrowh1" href="#" title="首页"  onclick="bdpPage('1','pageCheckFormId')"></a>
                            </li>
                            
                            <#if page.currentPageNo==1>
			           		 <li><a class="arrowh21" title="上一页" href="#" ></a></li>
				           	 <#else>
				       			<li ><a class="arrowh2" href="#" title="上一页" onclick="bdpPage('${page.previousPageNo}','pageCheckFormId')"></a></li>
				       		</#if>
                            ...
                            <li style="height: 30px; line-height:30px;">
				            	...
				            	<input id="gotoPageFormId" type="hidden" value="pageCheckFormId"/>
                            ...
                            <#if hNext >
				       			<li ><a class="arrowh3" title="下一页"  href="#" onclick="bdpPage('${page.nextPageNo}','pageCheckFormId')"> </a></li>
				           	<#else>
				           		<li><a class="arrowh31" title="下一页" href="#" > </a></li>
				       		</#if>
                            
                            <li >
                            <a href="#" class="arrowh4" title="尾页" onclick="bdpPage('${page.totalPageCount}','pageCheckFormId')">
                            </a>
                            </li>
                        </ul>
                    </div>
...
<div id="pageFormTeacherCheckDiv" name="pageFormTeacherCheckDiv" style="display:none">
</div>
<script >
$("document").ready(function(){
	...
	pageForm.id="pageCheckFormId";
	document.getElementById("pageFormTeacherCheckDiv").appendChild(pageForm);	
	
	$("#pageCheckFormId").append($("<input name='formId' type='hidden' value='${formId!""}'/>"));
	$("#pageCheckFormId").append($("<input name='selectedStudentId' type='hidden' value='${selectedId!""}'/>"));
	$("#pageCheckFormId").append($("<input name='name' type='hidden' value='${teacher.name!""}'/>"));
	$("#pageCheckFormId").append($("<input name='org.id' type='hidden' value='${teacher.org.id!""}'/>"));
	$("#pageCheckFormId").append($("<input name='id' type='hidden' value='${teacher.id!""}'/>"));
	$("#pageCheckFormId").append($("<input name='queryFlag' type='hidden' value='${queryFlag!""}'/>"));
	
	$("#pageFormTeacherCheckDiv").append($("#pageCheckFormId"));
		...
		var pageFormOptions = {
            ...
            success: function(data){
             	$('input[name="row_teacher_multi"]').each(function(){
					...
					var userTree = $.fn.zTree.getZTreeObj("_selectedTeachers");
	...
	$("#pageCheckFormId").ajaxForm(pageFormOptions);		
	...
	$("#_radioSelectTeacherPage #gotoPageNumber").keypress(function(event){
        ...
        if(curKey == 13){
        	...
        	if (!patrn.exec(toPageNo)){
        		...
        		bdpPage(toPageNo,"pageCheckFormId");
        	}
