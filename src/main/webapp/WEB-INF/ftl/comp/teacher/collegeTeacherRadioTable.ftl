<table class="table table-bordered table-striped tablecut" id="selectUserTable">
	<thead>
		<tr>
			<th class="table_checkbox" width="5%"></th>
			<th  width="15%">姓名</th>
			<th  width="15%">工号</th>
			<th  width="10%">性别</th>
			<th width="25%">学院</th>
	...
	<#if page??>
		<#list page.result as teacher>
			<tr>
			    <td><input type="radio" name="row_sel"  value="${(teacher.teacherInfo)!""}" <#if selectedTeacherId?? && selectedTeacherId == teacher.id>checked="true"</#if> onclick="_onClickCollegeTeacherRadio(this)"/></td>
				...
				<td class="autocut">${(teacher.nativePlace)!""}</td>
			</tr>
		</#list> 
	</#if>															
	</tbody>
...
<div class="row-fluid">
<div class="span12" style="margin-top:-19px;" id="_radioSelectCollegeTeacherPage">
	<div class="nextP">
...
$("document").ready(function(){
	...
	pageForm.action="${rc.contextPath}/comp/taecher/nsm/queryRadioCollegeTeacher.do";
	...
	$("#pageFormId").append($("<input name='selectedTeacherId' type='hidden' value='${selectedTeacherId!""}'/>"));
	$("#pageFormId").append($("<input name='name' type='hidden' value='${teacher.name!""}'/>"));
	$("#pageFormId").append($("<input name='org.id' type='hidden' value='${teacher.org.id!""}'/>"));
	$("#pageFormId").append($("<input name='code' type='hidden' value='${teacher.code!""}'/>"));
	...
	$("#_radioSelectCollegeTeacherPage #gotoPageNumber").keypress(function(event){
	
		var maxPageNo= $("#_radioSelectCollegeTeacherPage #gotoPageTotalPageCount").val()*1;
		var toPageNo= $("#_radioSelectCollegeTeacherPage #gotoPageNumber").val()*1;
