<table class="table table-bordered table-striped tablecut" id="selectUserTable">
	...
	<#if page??>
		<#list page.result as student>
			<tr>
			    <td><input type="radio" name="row_sel"  value="${(student.studentInfo)!""}" <#if studentId?? && studentId == student.id>checked="true"</#if> onclick="_onClickStudentRadio(this)"/></td>
...
$("document").ready(function(){
	...
	pageForm.action="${rc.contextPath}/comp/student/nsm/queryRadioSteudent.do";
	...
	$("#pageFormId").append($("<input name='selectedStudentId' type='hidden' value='${studentId!""}'/>"));
	...
	$("#pageFormId").append($("<input name='queryFlag' type='hidden' value='radio'/>"));
