...
<form id="statisticsQuery" method="post" action="${rc.contextPath}/job/replace/opt-query/statisticsRegisterByCollege.do">
<div class="span12">	
	<div class="row-fluid">
		<div class="span4">
			...
			<option value="" >请选择</option>
				<#if yearList??>
				<#list yearList as y>
					<#if registerStatistics?? && registerStatistics.dicYear ?? && registerStatistics.dicYear.id == y.id>
		...
		<div class="span4">
			...
			<select size="1" id="range" name="range" aria-controls="dt_gal" class="span7" onchange="statistics();">
				<option value="1" <#if registerStatistics?? && registerStatistics.range ?? && registerStatistics.range == '1'> selected="selected"</#if> >学院</option>
				<option value="2" <#if registerStatistics?? && registerStatistics.range ?? && registerStatistics.range == '2'> selected="selected"</#if> >专业</option>
				<option value="3" <#if registerStatistics?? && registerStatistics.range ?? && registerStatistics.range == '3'> selected="selected"</#if> >班级</option>
			</select>
		</div>
		    ...
		    <select size="1" id="collegeId" name="college.id" aria-controls="dt_gal" class="span6" onchange="cascade.changeCollage('collegeId','majorId');" >
			    ...
			    <#if collegeList??>
					<#list collegeList as c>
						<#if registerStatistics?? && registerStatistics.college?? && registerStatistics.college.id == c.id >
	...
	<div class="row-fluid">
		<div class="span4" >
		    ...
		    <select  class="span7" size="1" id="majorId" name="major.id" aria-controls="dt_gal" onChange="cascade.changeMajor('majorId','classId');" disabled="disabled" >
		    <option value="" >请选择</option>
		    <#if majorList ?? >
				<#list majorList as m>
					<#if registerStatistics ?? && registerStatistics.major?? && registerStatistics.major.id == m.id >
		...
		<div class="span4">
			...
			<select class="span7" size="1" id="classId" name="classId.id" aria-controls="dt_gal" disabled="disabled" >
				<option value="" >请选择</option>
				<#if classList ?? >
					<#list classList as c>
						<#if registerStatistics?? && registerStatistics.classId?? && registerStatistics.classId.id == c.id >
...
<script>
	...
	function statisticsRegister() {
		$("#statisticsQuery").attr("action", "${rc.contextPath}/job/replace/opt-query/statisticsRegisterByCollege.do"); 
	...
	$("#export-button").live("click", function() {
		...
		$("#export_statistics").load("${rc.contextPath}/job/register/nsm/exportRegisterList.do?pageTotalCount="+pageTotalCount+"&exportSize="+exportSize);
	});
	
	//点击导出按钮弹出的对话框中的链接，进行导出操作
	function exportDate(exportSize,exportPage){
		$("#statisticsQuery").attr("action", "${rc.contextPath}/job/register/opt-export/exportRegisterList.do");
