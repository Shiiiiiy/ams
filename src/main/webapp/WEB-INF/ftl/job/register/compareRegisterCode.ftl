...
<#if importFlag?? && page??>
	<form id="comparePageQuery" action="${rc.contextPath}/job/register/opt-import/importConfirmData.do" method="post">
		<div class="formSep">
			<div class="row-fluid">
				...
				<table class="table table-bordered table-striped tablecut" id="smpl_tbl">
					...
					<tbody id="demo_list">
						<#list page.result as p>
							...
							<td class="autocut">${(p[0].registerCode)!""?html}</td>
							<td class="autocut">${(p[1].stuName)!""?html}</td>
							<td class="autocut">${(p[1].stuNumber)!""?html}</td>
							<td class="autocut">${(p[1].registerCode)!""?html}</td>
							</tr>
						</#list>
					</tbody>
				</table>
