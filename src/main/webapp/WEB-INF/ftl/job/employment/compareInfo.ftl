<#if importFlag?? && page??>
<form  id="comparePageQuery" action="${rc.contextPath}/job/employment/opt-query/compareData.do" method="post">
	<div class="formSep">
		<div class="row-fluid">
				...
				<tbody id="demo_list">
					<#list page.result as p>
						<tr>
							...
							<td class="autocut">${(p[0].difficultType.name)!""}</td>
							<td class="autocut">${(p[1].student.name)!""?html}</td>
							<td class="autocut">${(p[1].student.stuNumber)!""?html}</td>
							<td class="autocut">${(p[1].difficultType.name)!""}</td>
						</tr>
					</#list>
				</tbody>
			</table>
...
<script type="text/javascript">
	...
	function gotoInspectPage(page) {
		jQuery.getJSON("${rc.contextPath}/job/employment/opt-query/employmentPageQuery.do",
			{
				"pageNo":page
			},function(data){
