	               ...
	               <form  id="paperEditForm" name="paperEditForm" method="post">
						<div class="span12">
							...
							<div class="btnLeft">
				              ...
				              <a href="javascript:void(0)" class="btn btn-info"  onclick="cancelCurPaper()">取 消</a>
				            </div>
				   </form>
			   </div>
            </div>
       ...
       <script>
			...
			var formValidator = $("#paperEditForm").compValidate({
       		...
       		function cancelCurPaper(){
       			$("#paperEditForm").attr("action","${rc.contextPath}/question/quesWrite/opt-edit/cancelCurPaper.do").submit();
       		}
       		...
       		function saveCurPaper(){
       			...
       			if(singleValidator && mulValidator && subValidator){
       				if(formValidator.form()){
		       			$("#paperEditForm").attr("action","${rc.contextPath}/question/quesWrite/opt-edit/saveCurPaper.do").submit();
       				}
       			}else{
       		...
       		function submitCurPaper(){
       			...
       			if(singleValidator && mulValidator && subValidator){
       				if(formValidator.form()){
	       				$("#paperEditForm").attr("action","${rc.contextPath}/question/quesWrite/opt-edit/submitCurPaper.do").submit();
	       			}
       			}else{
       				comp.message("请完成必答题 !","error");
       			}
