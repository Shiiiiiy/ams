...
<div id="contentwrapper">
	<div class="main_content">
		<div class="row-fluid">
			<form id="collegeList" action="${rc.contextPath}/leave/college/opt-query/queryCollegePage.do" method="post">
				<div class="span12">
					<div class="row-fluid">
						<div class="span4">
					...
					<div class="row-fluid">
						<div class="span4">
							...
							<select name="status" size="1" aria-controls="dt_gal" class="span7">
								...
								<#list stautsDic as hs>
									<#if leave?? && leave.status?? && leave.status == hs.id>
						...
						<div class="span4" style="padding-left:35px;">
							<button class="btn btn-info" type="button" onClick="queryCollege();">查 询</button>
						...
						<table class="table table-bordered table-striped tablecut" id="smpl_tbl" >
							<thead>
								<tr>
									...
									<th width="9%">学号</th>
									<th width="9%">办理年份</th>
									<th width="9%">图书信息中心</th>
									<th width="9%">公寓管理中心</th>
									<th width="8%">学费</th>
									<th width="9%">校园一卡通</th>
									<th width="9%">水电费</th>
									<th width="9%">党团关系</th>
								...
								<#if page??>
									<#list page.result as p>
										<tr>
											...
											<input type="hidden" id="${p_index+1}" name="${p_index+1}" value="${(p.student.stuNumber)!""}">
											...
											<td class="autocut">
												<#if p?? && p.status?? && p.college.handleStatus.code=="HANDLE" && user_key.optMap['update']??>
													<a href="###" title="取消办理" onclick="cancelCollege('${p.id}','UNHANDLE')"><i class="icon-ban-circle"></i></a>
												<#else>
													<#if user_key.optMap['update']??>
														<a href="###" title="办理" onclick="viewLeaveInfo('${p.id}')"><i class="icon-share"></i></a>
													</#if>
												</#if>
											</td>
										</tr>
...
<div class="modal hide fade" id="student_view_div" style="z-index:99999">
	...
	<div class="modal-footer">
		...
		<a href="###" class="btn btn-info" onclick="operateCollege($('#studentId').val(),'HANDLE')">确 定</a>
...
function brushIDCard(cardID){//刷身份证
	...
	$("#student_view_body").load("${rc.contextPath}/leave/college/nsm/queryStudentLeave.do?.temp="+Math.random(),
	{"studentId":$("#1").val()},
	//{"cardID":cardID},
	function(){
	});
...
//学生宿舍信息查看
function viewLeaveInfo(studentId) {
	$("#studentId").val(studentId);
	$("#student_view_body").empty();
	comp.showModal("student_view_div","700px","-300px 0 0 -380px"); 
	$("#student_view_body").load("${rc.contextPath}/leave/college/nsm/queryStudentLeave.do?.temp="+Math.random(),
	{"studentId":studentId},
	function(){
	});
...
//办理、取消办理
function operateCollege(studentId, command){
	$.post(
		"${rc.contextPath}/leave/college/opt-edit/operateCollege.do",
		{studentId: studentId, command:command},
		function(data){
			$("#collegeList").attr("action","${rc.contextPath}/leave/college/opt-query/queryCollegePage.do?pageNo="+$("#currentPageNo").val());
			$("#collegeList").submit();
		},
		"text"
	);	
...
function cancelCollege(studentId, command){
	var str="确认要取消办理吗？";
	comp.confirm(str, function(result){
		if(!result){
			return; }
		operateCollege(studentId, command);
	});
}
