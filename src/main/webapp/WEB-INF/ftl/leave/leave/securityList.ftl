...
<div id="contentwrapper">
	<div class="main_content">
		<div class="row-fluid">
			<form id="leaveList" action="${rc.contextPath}/leave/security/opt-query/querySecurityPage.do" method="post">
				<div class="span12">
					<div class="row-fluid">
						<div class="span4">
						...
						<div class="row">
							<div class="span6">
								<div class="dt_actions">
									<div class="row-fluid">
										<button class="btn btn-info" type="button" onclick="selectStudentCheck();">选择学生</button>
									</div>
								</div>
							</div>
						</div>
							...
							<tbody>
								<#if page??>
									...
									<tr>
										...
										<td class="autocut"><#if p.security=='1'>未办理<#elseif p.security=='2'>已办理</#if></td>
										<td class="autocut"><#if p.status=='0'>未发起<#elseif p.status=='1'>发起<#else>办结</#if></td>
										<#if user_key.optMap['update']??>
										<td class="autocut" name=${p.id!""?html}_td>
										<#if p.status=='1' && p.security=='1'>
											<a href="###"  onclick="operate('${p.id}','2')" title="办理"><i class="icon-share"></i></a>
										<#elseif p.status=='1' && p.security=='2'>
											<a href="###"  onclick="operate('${p.id}','1')" title="取消办理"><i class="icon-ban-circle"></i></a>
										</#if>
										</td>
										</#if>
									</tr>
...
<#-- 学生选择多选组件的引入-->
<#include "/leave/leave/queryStudentAllCheckBoxModal.ftl"> 
<#---脚本开始----->
<script>
	function queryLeave(){
		$("#leaveList").submit();
	...
	function operate(id, status){
		...
		comp.confirm(str, function(result) {
			...
			$.ajax({
				url:'${rc.contextPath}/leave/security/opt-edit/operateSecurity.do',
				async:false,
				cache: false,
				type: "POST",
				...
				success: function(data){
					if("success" == data){
						$("#leaveList").attr("action","${rc.contextPath}/leave/security/opt-query/querySecurityPage.do?pageNo="+$("#currentPageNo").val());
						$("#leaveList").submit();
					}else{
						comp.message("操作失败");
					}
	...
	function setMultiEduClassSelectValue(studentIds,names){
		$.ajax({
			url:'${rc.contextPath}/leave/security/opt-edit/operateSecurityStudent.do',
			async:false,
			cache: false,
			type: "POST",
