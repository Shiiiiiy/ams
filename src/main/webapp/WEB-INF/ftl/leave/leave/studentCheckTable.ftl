<table class="table table-bordered table-striped tablecut" id="selectUserTable">
	...
	<#if page??>
		<#list page.result as student>
			<tr>
			    <td><input type="checkbox" name="row_sel_multi"  value="${(student.studentInfo)!""}" <#if selectedId?? && selectedId == student.id>checked="true"</#if> onclick="_onClickEduClassCheckbox(this)"/></td>
