<#if importFlag?? && page??>
<form  id="comparePageQuery" action="${rc.contextPath}/demo/demo/opt-import/importData.do" method="post">
	<div class="formSep">
		<div class="row-fluid">
				...
				<tbody id="demo_list">
					<#list page.result as p>
						<tr>
							...
							<td class="autocut">${(p[0].name)!""?html}</td>
							<td class="autocut">${(p[0].code)!""?html}</td>
							<td class="autocut">${(p[0].gender.name)!""?html}</td>
							<td class="autocut">${(p[1].name)!""?html}</td>
							<td class="autocut">${(p[1].code)!""?html}</td>
							<td class="autocut">${(p[1].gender.name)!""?html}</td>
						</tr>
					</#list>
				</tbody>
			</table>
...
<script type="text/javascript">
	...
	function gotoInspectPage(page) {
		jQuery.getJSON("${rc.contextPath}/demo/demo/opt-query/pageQuery.do",
			{
				"pageNo":page
			},function(data){
				...
				jQuery.each(result, function(i, item) {
					...
					var html = "<tr>"
							...
							+"	<td class='autocut'>"+item.code+"</td>"
							+"	<td class='autocut'>"+item.gender+"</td>"
							+"	<td class='autocut'>"+item.excelName+"</td>"
							+"	<td class='autocut'>"+item.excelCode+"</td>"
							+"	<td class='autocut'>"+item.excelGender+"</td>"
							+"</tr>";
