<#if importFlag?? && page??>
<form  id="comparePageQuery" action="${rc.contextPath}/warning/importPSYCHOLOGY/opt-query/comparePsychologyInfo.do" method="post">
	<div class="formSep">
		<div class="row-fluid">
				...
				<tbody id="demo_list">
					<#list page.result as p>
						<tr>
							...
							<td class="autocut">${(p[0].onlyChild.name)!""?html}</td>
							<td class="autocut">${(p[0].childFoster.name)!""}</td>
							<td class="autocut">${(p[1].student.stuNumber)!""?html}</td>
							<td class="autocut">${(p[1].onlyChild.name)!""?html}</td>
							<td class="autocut">${(p[1].childFoster.name)!""}</td>
						</tr>
					</#list>
				</tbody>
			</table>
...
<script type="text/javascript">
	...
	function gotoInspectPage(page) {
		jQuery.getJSON("${rc.contextPath}/warning/importPSYCHOLOGY/opt-query/psychologyPageQuery.do",
			{
				"pageNo":page
			},function(data){
				...
				jQuery.each(result, function(i, item) {
					...
					var html = "<tr>"
							...
							+"	<td class='autocut'>"+item.name+"</td>"
							+"	<td class='autocut'>"+item.stuNumber+"</td>"
							+"	<td class='autocut'>"+item.difficultType+"</td>"
							+"	<td class='autocut'>"+item.excelName+"</td>"
							+"	<td class='autocut'>"+item.excelStuNumber+"</td>"
							+"	<td class='autocut'>"+item.excelDifficultType+"</td>"
							+"</tr>";
