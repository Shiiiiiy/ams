...
//增加节点
		function addOrgNode_(){
			...
			var namePatrn=/^[\u0391-\uFFE50-9a-zA-Z_]*$/; 
			if(name==""||name==null){
				validateFormat("name","组织机构名称不能为空");
				flage=true
			}else if(name.length>30){
				validateFormat("name","组织机构名称只能是长度1-30");
				flage=true
			}else if(!namePatrn.exec(name)){
				validateFormat("name","组织机构名称只能是中文，数字，英文，下划线");
				flage=true
			}
			var patrn=/^[A-Za-z0-9]{1,20}$/; 
			if(code==""||code==null){
				validateFormat("code","组织机构编码不能为空");
				flage=true
			}else if (!patrn.exec(code)){
				validateFormat("code","组织机构编码只能是长度1-20的数字,英文");
				flage=true;
			}
			if(area.length>20){
				validateFormat("area","区域最大长度不能超过20");
				flage=true;
			}
			if(comments.length>1500){
				validateFormat("comments","备注最大长度不能超过1500");
				flage=true;
			}
			if(flage){
				return false;
			...
			var textError=isExistMenuName(name,code,pnode.id);
			var json = eval(textError);
			var f=false;
			if(json[0].name!="success"){
				validateFormat("name",json[0].name);
				f=true;
			}
			if(json[0].code!="success"){
				validateFormat("code",json[0].code);
				f=true;
			}
			if(f){
				return false;
...
//编辑菜单
		function editOrgNode(){
			...
			var namePatrn=/^[\u0391-\uFFE50-9a-zA-Z_]*$/; 
			if(name==""||name==null){
				validateFormat("name_","组织机构名称不能为空");
				flage=true;
			}else if(name.length>30){
				validateFormat("name_","组织机构名称只能是长度1-30");
				flage=true
			}else if(!namePatrn.exec(name)){
				validateFormat("name_","组织机构编码只能是中文，数字，英文，下划线");
				flage=true
			}
			
			if(code==""||code==null){
				validateFormat("code_","组织机构编码不能为空");
				flage=true;
			}
			var patrn=/^[A-Za-z0-9]{1,20}$/; 
			if (!patrn.exec(code)){
				validateFormat("code_","组织机构编码只能是长度1-20的数字,英文");
				flage=true;
			}
			if(area.length>20){
				validateFormat("area_","区域最大长度不能超过20");
				flage=true;
			}
			if(comments.length>1500){
				validateFormat("comments_","备注最大长度不能超过1500");
				flage=true;
			}
			if(flage){
				return;
			}
			var textError=isExistMenuName(name,code,pId,id);
			var json = eval(textError);
			var f=false;
			if(json[0].name!="success"){
				validateFormat("name_",json[0].name);
				f=true;
			}
			if(json[0].code!="success"){
				validateFormat("code_",json[0].code);
				f=true;
			}
			if(f){
				return false;
...
//调整
        function moveOrg(){
			...
			var textError=isExistMenuName(curNode.name,null,targetNodes[0].id,id);
			var json = eval(textError);
			var f=false;
			if(json[0].name!="success"){
				$.sticky("同级组织机构名称有重复，不能移动到该组织机构下下", {autoclose : 5000, position: "top-right", type: "st-error" });
				return false;
			}
...
//初始化树
		...
		//校验菜单同级不能重名
		function isExistMenuName(name,code,pid,id){
			var flag;
			$.ajax({
					async : false,
					cache:false,
					url: "${rc.contextPath}/user/org/opt-query/checkData.do",
					data:{name:name,code:code,pid:pid,id:id},
					type: "POST",
					error: function () {
						$.sticky("请求失败，请稍后再试!", {autoclose : 5000, position: "top-right", type: "st-error" });
					},
