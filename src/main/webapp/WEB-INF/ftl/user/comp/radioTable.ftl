...
<table class="table table-bordered table-striped tablecut" id="selectUserTable">
							<thead>
								<tr>
									<th class="table_checkbox" width="2%"></th>
									<th width="30%">编号</th>
									<th  width="25%">姓名</th>
									<th width="20%">类型</th>
									<th width="15%">性别</th>
							...
							<#if page??>
								<#list page.result as user>
									<tr>
									    <td><input type="radio" name="row_sel" value="${(user.userInfo)!""}" onclick="_onClickRadio(this)"/></td>
										<td class="autocut">${(user.id)!""}</td>
										<td class="autocut">${(user.name)!""}</td>
										<td class="autocut">${(user.userType.name)!""}</td>
										<td class="autocut">${(user.genderDic.name)!""}</td>
									</tr>
								</#list> 
							</#if>															
							</tbody>
...
$("document").ready(function(){
	...
	$("#pageFormId").append($("<input name='tag' type='hidden' value='radio'/>"));
		...
		var pageFormOptions = {
            ...
            success: function(data){
			    ...
			    $('input[name="row_sel"]').each(function(){
					...
					if($("#_userInfo").val()==''&&obj[0].userId==$("#_userId").val()){		
					...
					if($("#_userInfo").val()=='')
						return;					
					var userInfo=eval("["+$("#_userInfo").val()+"]");
					if(obj[0].userId==userInfo[0].userId)
						radio.attr("checked",true);					
			    });	
