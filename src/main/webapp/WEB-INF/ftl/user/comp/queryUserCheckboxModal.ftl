...
<div class="modal hide fade" id="_selectUserModal">
	...
	<div class="modal-body" id="selectUserModalBody">
		<div class="row-fluid">
			<div class="span3">
				<ul id="_selectUserOrgTree_" class="ztree myztree"></ul>
			</div>
			<div class="span6">
				<div class="row-fluid">
						<div class="span12">
							...
							<input id="selectUserType" name="selectUserType" type="hidden" value="userName"/>
...
<script>
	...
	function _queryUsersByOrgId(event, treeId, treeNode){
		var treeObj = $.fn.zTree.getZTreeObj("_selectUserOrgTree_");
		var node = treeObj.getNodes()[0];
		if(treeNode.id!=node.id && treeNode.id !="000")//组机构根节点不传递 组织机构id给后台
	...
	function _initValues(ids,status){
		...
		$('#selectUserModalBody').css("max-height","420px");
		$('#_selectUserOrgTree_').width(200).height(320).css("margin-top","0px");
		...
		//初始化组织机构树
		comp.initTree("_selectUserOrgTree_",comp.contextPath()+"/user/comp/getOrgTree.do",null,null,_queryUsersByOrgId);
