<!--学生选择 -->
<div class="modal hide fade" id="_selectStudentVolunteerAllCheckModal">
	<div class="modal-header">
		<button class="close" data-dismiss="modal">×</button>
		<h3>志愿者选择</h3>
	</div>
	<div class="modal-body" id="_selectStudentVolunteerAllCheckModalBody">
	<div class="row-fluid">
	<div class="span9">
		<div class="row-fluid">
			<div class="span12">
				<form  id="_studentAllCheckQuery"  action="${rc.contextPath}/volunteer/volunteerRadio/nsm/queryRadioVolunteer.do" method="post">
					...
					<div class="row-fluid">
						<div class="span4">	
							学院	
							<select  disabled   aria-controls="dt_gal"  class="span8" onchange="cascade.changeCollage('_checkCollegeId','_checkMajorId','_checkClassId');">
									<option value="${teacherOrgId}" selected="selected">${orgName}</option>
							</select>	
							<input type="hidden" id="_checkCollegeId" name="stuInfo.college.id" value="${teacherOrgId}"/>
						</div>
						<div class="span4">	
							专业
							<select  id="_checkMajorId" name="stuInfo.major.id" aria-controls="dt_gal"  class="span8" onchange="cascade.changeMajor('_checkMajorId','_checkClassId');">
								<option value="">请选择..</option>
							</select>				
						</div>
						<div class="span4">	
							班级	
							<select  id="_checkClassId" name="stuInfo.classId.id" aria-controls="dt_gal"  class="span8">
								<option value="">请选择..</option>
							</select>				
						</div>
					</div>
					<div class="row-fluid">
						<div class="span4">	
							 姓名	
							<input id="_checkName" name="stuInfo.name" class="span8"  value=""/>
						</div>
						<div class="span4">	
							学号
							<input id="_checkStuNumber" name="stuInfo.stuNumber" class="span8"  value=""/>
						</div>
						<div class="span4">	
			...
			<div class="span3">
				<ul id="_selectedAllVolunteerStudent" class="ztree myztree"></ul>
...
//业务页面调用方法	
function _queryStudentVolunteerCheckAll(ids,flag)
{
	_initCheckValues(ids,flag);
	_queryStudentCheckSubmit();		
	comp.showModal("_selectStudentVolunteerAllCheckModal");
	cascade.changeCollage('_checkCollegeId','_checkMajorId','_checkClassId');
...
{
	$('#_selectStudentVolunteerAllCheckModal').width(890).css("marginLeft","-380px").css("marginTop","-250px");
	$('#_selectStudentVolunteerAllCheckModalBody').height(350).css("max-height","380px");
	$('#_selectedAllVolunteerStudent').width(200).height(300).css("margin-top","0px");
	$('#_studentCheckQueryButton').css("margin-bottom","9px");
	$('#_studentCheckClearButton').css("margin-bottom","9px");
	$("#_checkCollegeId").val("${teacherOrgId}");
	$("#_checkMajorId").val("");
	$("#_checkClassId").val("");
	$("#_checkName").val("");
	$("#_setFlag").val(flag);
	...
	$.ajax({
		...
		$.fn.zTree.init($("#_selectedAllVolunteerStudent"), _selectedAllClassStudentSet, calssStudentNodes);	
...
{
	var treeObj = $.fn.zTree.getZTreeObj("_selectedAllVolunteerStudent");
	if($("#_setFlag").val())
	{
