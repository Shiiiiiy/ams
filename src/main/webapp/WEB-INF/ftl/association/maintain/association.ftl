<#if importFlag?? && page??>
<form  id="associationMemberPageQuery" action="${rc.contextPath}/association/maintain/opt-query/importData.do" method="post">
	<div class="formSep">
		<div class="row-fluid">
				...
				<tbody id="demo_list">
					...
					<#if p[0].memberPo ??>
						<tr>
							...
							<td class="autocut" colspan="15">${p[0].memberPo.name!""?html}</td>
							<td class="autocut" colspan="15">${p[0].memberPo.stuNumber!""?html}</td>
                           	<td class="autocut" colspan="15">${p[0].memberPo.genderDic.name!""?html}</td>                            
						    <td class="autocut" colspan="15">${p[0].memberPo.name!""?html}</td>
							<td class="autocut" colspan="15">${p[0].memberPo.stuNumber!""?html}</td>
                           	<td class="autocut" colspan="15">${p[0].memberPo.genderDic.name!""?html}</td>
						</tr>
...
<script type="text/javascript">
	...
	function submitImport() {
		...
		comp.confirm("所选择的记录导入时将被忽略，确认导入吗？", function(flag) {
			if (flag)
				jQuery("#associationMemberPageQuery").submit();
		});
	}
	
	function gotoInspectPage(page) {
		jQuery.getJSON("${rc.contextPath}/association/maintain/opt-query/association.do",
			{
				"pageNo":page
			},function(data){
				...
				jQuery.each(result, function(i, item) {
					...
					var html = "<tr>"
							...
							+"	<td colspan='10' class='autocut'>"+item.genderName+"</td>"
							+"	<td colspan='8' class='autocut'>"+item.xlsStuName+"</td>"
							+"	<td colspan='10' class='autocut'>"+item.xlsStuNumber+"</td>"
							+"	<td colspan='10' class='autocut'>"+item.xlsGenderName+"</td>"
							+"</tr>";
