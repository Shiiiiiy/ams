...
<div id="contentwrapper">
    ...
    <div class="row-fluid">
		...
		<div class="span6">
        	<p class="btnMargin">
        		<button class="btn btn-info" type="button" onclick="onSub('CONFIRMED')">提 交</button>
...
<script>
	...
	//取消
	function cancel(){
		window.location.href="${rc.contextPath}/evaluation/update/opt-query/queryConfirmEvaluationList.do";
	}
	...
	function changeAll(num) { 
		...
		var sum=$('.scoreSum');
		var weigth=$('#WEIGTH');
		var baseScore=$('#BASESCORE'+num);
		var rate=0;
		
		$('.col'+num).each(function(){
			sumValue += Number($(this).val()); 
		});
		sumValue = transferToFlate(sumValue);
		$('#scores'+num).val(sumValue);
	} 
	
	
   // 数字转换 add 2016-01-07 修改 bug1102 添加
   function transferToFlate(numberVal) 
   {    
        var f = parseFloat(numberVal);    
        if (isNaN(f)) {    
            return false;    
        }    
        var f = Math.round(numberVal*100)/100;    
        var s = f.toString();    
        var rs = s.indexOf('.');    
        if (rs < 0) {    
            rs = s.length;    
