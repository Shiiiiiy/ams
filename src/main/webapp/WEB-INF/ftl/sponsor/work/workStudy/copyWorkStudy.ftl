		    ...
		    <div class="formSep"> 
			   <div class="row-fluid">
					...
					<div class="span6">
						...
						<div class="span9">
							<select  id="sourceTermId" size="1" id="term" name="sourceTermId" aria-controls="dt_gal"  class="{required:true,messages:{required:'学期不能为空'}}" onchange="changeTerm()">
									<option value="" >请选择</option>
									<#if termList??>
										<#list termList as t>
		   ...
		   <div class="formSep"> 
			   <div class="row-fluid">
					...
					<div class="span6">
						...
						<div class="span9">
							<select  id="aimTermId" size="1" id="term" name="aimTermId" aria-controls="dt_gal"  class="{required:true,messages:{required:'学期不能为空'}}" onchange="changeTerm()">
									<option value="" >请选择</option>
									<#if termList??>
										<#list termList as t>
			   ...
			   <div class="formSep">
					<div class="row-fluid">
						<div class="span6">
							...
							<div class="span9">
								<input id="studentId" type="hidden" name="studentId"  value="${(currentStudentInfo.id)!""} "/>
								<input id="stuName"  readOnly value="${(currentStudentInfo.name)!""}" />
							</div>
						</div>
				  	</div>
			   </div>
			...
			if(check_error == "true"){
			    if(stas=="0"){
	                ...
	                if(sourceSchoolYearId==aimSchoolYearId &&sourceTermId==aimTermId){
	                 ...
	                 $.ajax({
	                 	url:"${rc.contextPath}/sponsor/workStudy/opt-save/saveCopyWorkStudy.do",
	                 	async:false,
						cache:false,
						type:"POST",
						data:{sourceSchoolYearId:sourceSchoolYearId,sourceTermId:sourceTermId,aimSchoolYearId:aimSchoolYearId,aimTermId:aimTermId,studentId:studentId},
	                 	success:function(msg){
	                 		if("success"==msg){
	                 			...
	                 			comp.message("源学年学期审批通过的勤工助学信息不存在，请检查","error");
	                 		}else{
	                 			comp.message("目标学年学期勤工助学信息已经存在","error");
	                 		}
