<#if importFlag?? && page??>
<form  id="comparePageQuery" action="${rc.contextPath}/reward/helpaward/opt-query/importHelpData.do" method="post">
	<div class="formSep">
		<div class="row-fluid">
				...
				<tbody id="demo_list">
					<#list page.result as p>
						<tr>
							...
							<td class="autocut">${(p[0].helpGrade.name)!""?html}</td>
							<td class="autocut">${(p[0].stuId.name)!""?html}</td>
							<td class="autocut">${(p[0].stuId.id)!""?html}</td>
...
<script type="text/javascript">
	...
	function gotoInspectPage(page) {
		jQuery.getJSON("${rc.contextPath}/reward/helpaward/opt-query/compareHelpBurse.do",
			{
				"pageNo":page
			},function(data){
