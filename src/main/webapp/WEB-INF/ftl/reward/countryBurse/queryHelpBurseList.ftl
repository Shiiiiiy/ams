		...
		<script>
			...
			function del(id){//删除
				comp.confirm("确定要删除数据吗？",function(r){
					if(r){
						window.location.href="${rc.contextPath}/reward/helpaward/opt-query/deleteHelpBurse.do?id=" + id;
					}
				});
			}
			
			function view(id) {  //查看
				window.open("${rc.contextPath}/reward/helpaward/opt-query/viewHelpBurse.do?id="+id);
			}
			function importBurse() {
				window.location.href="${rc.contextPath}/reward/countryBurse/importHelpBurse.ftl";
			}
			...
			function delBurses(){
				...
				if(checkedIds == '' || checkedIds == null || checkedIds=='null'){
					...
					comp.confirm("确定要删除这几条数据吗？",function(r){
						if(r){
							window.location.href="${rc.contextPath}/reward/helpaward/opt-query/deleteHelpBurse.do?id=" + checkedIds;
						}
					});
				}
			}
    ...
    <body>
		<div id="contentwrapper">
		    <div class="main_content">  
		    	<div class="row-fluid">
		    		<form id="burseQuery" method="post" action="${rc.contextPath}/reward/helpaward/opt-query/queryHelpBursePage.do">
		    			<div class="span12">
		    				<div class="row-fluid">
		    					<div class="span4">
	           							...
	           							<#if page??>	
											<#list page.result as p>
												<tr>
													...
													<td class="autocut" >${p.helpGrade.name!""?html}</td>
													<td class="autocut">
													<#if user_key.optMap['del']??>
