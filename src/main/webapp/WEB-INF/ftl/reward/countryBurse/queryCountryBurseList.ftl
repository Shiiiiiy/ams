		...
		<script>
			...
			function del(id){//删除
				comp.confirm("确定要删除数据吗？",function(r){
					if(r){
						window.location.href="${rc.contextPath}/reward/countryaward/opt-query/deleteCountryBurse.do?id=" + id;
					}
				});
			}
			
			function view(id) {  //查看
				window.open("${rc.contextPath}/reward/countryaward/opt-query/viewCountryBurse.do?id="+id);
			}
			function importBurse() {
				window.location.href="${rc.contextPath}/reward/countryBurse/importCountryBurse.ftl";
			}
			...
			function delBurses(){
				...
				if(checkedIds == '' || checkedIds == null || checkedIds=='null'){
					...
					comp.confirm("确定要删除这几条数据吗？",function(r){
						if(r){
							window.location.href="${rc.contextPath}/reward/countryaward/opt-query/deleteCountryBurse.do?id=" + checkedIds;
						}
					});
				}
			}
    ...
    <body>
		<div id="contentwrapper">
		    <div class="main_content">  
		    	<div class="row-fluid">
		    		<form id="burseQuery" method="post" action="${rc.contextPath}/reward/countryaward/opt-query/queryCountryBursePage.do">
		    			<div class="span12">
		    				<div class="row-fluid">
		    					<div class="span4">
