<#if importFlag?? && page??>
<form  id="comparePageQuery" action="${rc.contextPath}/reward/inspireaward/opt-query/importInspireData.do" method="post">
	<div class="formSep">
		<div class="row-fluid">
				...
				<tbody id="demo_list">
					<#list page.result as p>
						<tr>
							...
							<td class="autocut">${(p[0].schoolYear.name)!""?html}</td>
							<td class="autocut">${(p[0].burseName.name)!""?html}</td>
							<td class="autocut">${(p[0].stuId.name)!""?html}</td>
							<td class="autocut">${(p[0].stuId.id)!""?html}</td>
							<td class="autocut">${(p[1].schoolYear.name)!""?html}</td>
							<td class="autocut">${(p[1].burseName.name)!""?html}</td>
						</tr>
					</#list>
				</tbody>
			</table>
...
<script type="text/javascript">
	...
	function gotoInspectPage(page) {
		jQuery.getJSON("${rc.contextPath}/reward/inspireaward/opt-query/compareInspireBurse.do",
			{
				"pageNo":page
			},function(data){
				...
				jQuery.each(result, function(i, item) {
					...
					var html = "<tr>"
							...
							+"	<td class='autocut'>"+item.year+"</td>"
							+"	<td class='autocut'>"+item.burseName+"</td>"
							+"	<td class='autocut'>"+item.xlsStuName+"</td>"
							+"	<td class='autocut'>"+item.xlsStuId+"</td>"
							+"	<td class='autocut'>"+item.xlsYear+"</td>"
							+"	<td class='autocut'>"+item.xlsBurseName+"</td>"
							+"</tr>";
