<#if importFlag?? && page??>
<form  id="comparePageQuery" action="${rc.contextPath}/reward/studentapprove/opt-query/importData.do" method="post">
	<div class="formSep">
		<div class="row-fluid">
				...
				<tbody id="demo_list">
					<#list page.result as p>
						<tr>
							...
							<td class="autocut">${(p[0].studentId.name)!""?html}</td>
							<td class="autocut">${(p[0].studentId.stuNumber)!""?html}</td>
							<td class="autocut">${(p[0].awardTypeId.awardInfoId.awardName)!""?html}</td>
							<td class="autocut">${(p[0].awardTypeId.secondAwardName.name)!""?html}</td>
							<td class="autocut">${(p[0].studentId.name)!""?html}</td>
							<td class="autocut">${(p[0].studentId.stuNumber)!""?html}</td>
							<td class="autocut">${(p[1].awardTypeIdStr)!""?html}</td>
							<td class="autocut">${(p[0].awardTypeId.secondAwardName.name)!""?html}</td>
						</tr>
					</#list>
				</tbody>
...
<script type="text/javascript">
	...
	function gotoInspectPage(page) {
		jQuery.getJSON("${rc.contextPath}/reward/studentapprove/opt-query/compareStudentApprove.do",
			{
				"pageNo":page
			},function(data){
				...
				jQuery.each(result, function(i, item) {
					...
					var html = "<tr>"
							...
							+"	<td class='autocut'>"+item.awardType+"</td>"
							+"	<td class='autocut'>"+item.secondAward+"</td>"
							+"	<td class='autocut'>"+item.xlsStuName+"</td>"
							+"	<td class='autocut'>"+item.xlsStuId+"</td>"
							+"	<td class='autocut'>"+item.xlsAwardType+"</td>"
							+"	<td class='autocut'>"+item.xlsSecondAward+"</td>"
							+"</tr>";
