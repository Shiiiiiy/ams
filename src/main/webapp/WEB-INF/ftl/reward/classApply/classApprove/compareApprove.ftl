<#if importFlag?? && page??>
<form  id="comparePageQuery" action="${rc.contextPath}/reward/classapprove/opt-query/importData.do" method="post">
	<div class="formSep">
		<div class="row-fluid">
			...
			<table class="table table-bordered table-striped tablecut" id="smpl_tbl">
				<thead>
					<tr>
						...
						<th width="45%" colspan="2" style="text-align:center;vertical-align:middle;">数据库数据</th>
						<th width="45%" colspan="2" style="text-align:center;vertical-align:middle;">Excel数据</th>
					</tr>
					<tr>
						<th width="10%">班级名称</th>
						<th width="15%">奖项名称</th>
				...
				<tbody id="demo_list">
					<#list page.result as p>
						<tr>
							...
							<td class="autocut">${(p[0].classId.className)!""?html}</td>
		...
		jQuery.getJSON("${rc.contextPath}/reward/managepunish/opt-query/comparePunish.do",
			{
				...
				jQuery.each(result, function(i, item) {
					...
					var html = "<tr>"
							...
							+"	<td class='autocut'>"+item.className+"</td>"
							+"	<td class='autocut'>"+item.awardName+"</td>"
							+"	<td class='autocut'>"+item.xlsClassName+"</td>"
							+"	<td class='autocut'>"+item.xlsAwardName+"</td>"
							+"</tr>";
