    ...
    <head>
		...
		<script>
			...
			function copyFileIdInput(){
				$("#classApplyEdit").append($("#fileUploadForm input[name='fileId']"));
			}
			
			//返回
			function returnPunishList(){   
				window.location.href="${rc.contextPath}/reward/classapply/opt-query/queryClassApplyPage.do";
			}
			...
			function onSub(stas){
				if(attachFlag.form()==false){
		 		...
		 		$("#flags").val(stas);
		 		var classInfoId = $("#classApplyInfoId").val();
				if(check_error == "true"){
				   if(stas=="1"){
	                  	if(confirm("提交信息后不能修改，确认要提交吗？")){
                  			copyFileIdInput();
                  			$.ajax({
								url: "${rc.contextPath}/reward/classapply/opt-query/saveClassApplyJson.do",
								async:false,
								cache:false,
								type:"POST",
								data:$('#classApplyEdit').serialize(),
							    success: function(data){
									if(data!=null && data!=""){
								       apw.selectNextApprover(data,"INIT","REWARD_CLASS_APPLY_APPROVE","");	
								    }else{
								    	$.sticky("班级信息保存失败", {autoclose:5000, position:"top-right", type:"st-error"});
								    	return;
								    }
								}
							});
			...
			function doInitAction(objectId,userId){
			    var url = "${rc.contextPath}/reward/classapply/opt-add/saveCurProcess.do";
			...
			$("document").ready(function(){
           		attachFlag = $("#classApplyEdit").compValidate({
					rules:{
						'condition.comments':{maxlength:500},
						classAwardInfo :{required:true,maxlength:500},
						mainResult :{required:true,maxlength:500}
					},
					messages:{
						'condition.comments':{maxlength:'最多只能输入500字'},
						classAwardInfo :{required:'请填写班级获奖情况',maxlength:'最多只能输入500字'},
						mainResult :{required:'请填写主要成绩',maxlength:'最多只能输入500字'}
					}
				});
			});
		...
		<div id="contentwrapper">
		    <div class="main_content">
		    	<div class="row-fluid">
					<div class="span12">
						<#if classInfo?? && classInfo.id??>
							<h3 class="heading">班级评优申请修改页面</h3>
						<#else>
							<h3 class="heading">班级评优申请页面</h3>
						</#if>
					</div>
				</div>
		    	...
		    	<div class="row-fluid">
						...
						<div class="row-fluid">
							<div class="span12">
								<h4 class="heading">班级信息</h3>
							</div>
						</div>
						<form id="classApplyEdit" method="post" action="${rc.contextPath}/reward/classapply/opt-query/saveClassApply.do">
						<#-- 对象ID -->
		    			<input type="hidden" id="classApplyInfoId" name="id" value="${classInfo.id!""}">
		    			<input type="hidden" id="awardTypeId" name="awardId" value="${classInfo.awardTypeId.id!""}">
		    				...
		    				<div class="row-fluid">
		    					<div class="span6">
		    						...
		    						<div class="span4">
				    					...
				    					<input type="hidden" name="classId.id" value="<#if classInfo.classId??>${classInfo.classId.id!""}</#if>" readonly />
		    						</div>
		    					</div>
		    					<div class="span6">
		    						<div class="span4">
		    				    	...
		    				    	<div class="span4">
										<input id="studentId.id" name="classId.headermaster.name" value="<#if classInfo.classId??>${classInfo.classId.headermaster.name!""}</#if>" readonly>
									</div>
	    				    	</div>
		    				</div>
	    				</div>
						<div class="formSep">
		    				<div class="row-fluid">
		    					<div class="span6">
		    						<div class="span4">
		    				    		<span>班级获奖情况<span class="f_req">*</span></span>
		    						</div>
		    				    	<div class="span4">
										<textarea id="classAwardInfo" name="classAwardInfo" col="12" rows="3" style="width: 770px; height: 111px;">${classInfo.classAwardInfo!""}</textarea>
										</br>
		    			...
		    			<div class="formSep">
		    				<div class="row-fluid">
		    					<div class="span6">
		    						<div class="span4">
		    				    		<span>主要成绩<span class="f_req">*</span></span>
		    						</div>
		    				    	<div class="span4">
										<textarea id="mainResult" name="mainResult" col="12" rows="3" style="width: 770px; height: 111px;">${classInfo.mainResult!""}</textarea>
										</br>
										<font size="1" color="grey"  style="vertical-align:bottom;">(描述不超过500字)</font>
									</div>
	    				    	</div>
						...
						<div class="span6">
				    		...
				    		<div class="span8">
							    ...
							    <button class="btn btn-info" type="button" id="submit-button" onclick="onSub('1')">提 交</button>
					  			...
					  			<#if classInfo.processStatus?? && classInfo.processStatus == 'REJECT'>
